...
enum PackStreamInner {
    ...
    Unknown {
        ...
        /// The engine flavor to use when parsing the pack.
        flavor: EngineFlavor,
...
impl PackStream {
    ...
    pub fn new(path: PathBuf, file: File, flavor: EngineFlavor) -> PackStream {
                ...
                match file.read(buf) {
                    ...
                    Ok(n) => {
                        ...
                        if let Some(possible_magic) = possible_magic
                            && possible_magic == flavor.pck_header_magic
                        {
                            file.seek(SeekFrom::Start(offset))?;
                            match Pack::parse(&mut file, &flavor) {
                                Ok(pack) => {
...
impl StreamFactory for GDPatchStreamFactory {
    fn create_stream(&mut self, path: &Path) -> io::Result<Option<Box<dyn Stream>>> {
        ...
        if asked_for_pck || asked_for_exe {
            ...
            return Ok(Some(Box::new(PackStream::new(
                path,
                file,
                self.flavor.clone(),
            ))));
